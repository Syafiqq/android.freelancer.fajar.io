...
public class CConstitution extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener
{
    public static final String CLASS_NAME = "CConstitution";
    public static final String CLASS_PATH = "io.localhost.freelancer.statushukum.controller.CConstitution";
    ...
    private void setProperty()
    {
        ...
        this.search.setOnQueryTextListener(new SearchView.OnQueryTextListener()
        {
            ...
            {
                if((query.trim().length() > 0) && (!CConstitution.this.latestQuery.contentEquals(query)))
                {
                    CConstitution.this.latestQuery = query;
                    CConstitution.this.doSearch(query);
                }
                return false;
            }
            ...
            {
                CConstitution.this.searchAdapter.getFilter().filter(newText);
                return false;
            }
        });
        this.search.setOnQueryTextFocusChangeListener(new View.OnFocusChangeListener()
        {
            ...
            public void onFocusChange(View view, boolean isOnFocus)
            {
                ...
                if(isOnFocus)
                {
                    CConstitution.this.yearListView.setVisibility(View.GONE);
                    CConstitution.this.searchListView.setVisibility(View.VISIBLE);
                }
                else
                {
                    CConstitution.this.yearListView.setVisibility(View.VISIBLE);
                    CConstitution.this.searchListView.setVisibility(View.GONE);
                }
            }
        });
    }
    ...
    private void doSearch(final String query)
    {
        ...
        new AsyncTask<Void, Void, Void>()
        {
            ...
            {
                final MDM_Data modelData = MDM_Data.getInstance(CConstitution.this);
                final MDM_DataTag modelDataTag = MDM_DataTag.getInstance(CConstitution.this);
                final MDM_Tag modelTag = MDM_Tag.getInstance(CConstitution.this);
                ...
                CConstitution.this.searchList.clear();
                CConstitution.this.searchList.addAll(dbResultData);
                CConstitution.this.searchAdapter.update(dbResultData);
                return null;
            }
            ...
            {
                if(CConstitution.this.searchList.size() == 0)
                {
                    Toast.makeText(CConstitution.this, CConstitution.super.getResources().getString(R.string.activity_search_info_search_empty), Toast.LENGTH_SHORT).show();
                }
                CConstitution.this.searchAdapter.notifyDataSetChanged();
    ...
    private synchronized void setYearList()
    {
        ...
        new AsyncTask<Void, Void, Void>()
        {
            ...
            protected Void doInBackground(Void... voids)
            {
                ...
                CConstitution.this.yearList.clear();
                CConstitution.this.yearList.addAll(dbResult);
                CConstitution.this.yearAdapter.update(dbResult);
                return null;
            }
            ...
            protected void onPostExecute(Void aVoid)
            {
                CConstitution.this.yearAdapter.notifyDataSetChanged();
                super.onPostExecute(aVoid);
            }
        }.execute();
    }
    ...
    public boolean onOptionsItemSelected(MenuItem item)
    {
        ...
        switch(item.getItemId())
        {
            ...
            {
                CConstitution.this.onBackButtonPressed();
                return true;
            }
        }
    ...
    public boolean onNavigationItemSelected(@NonNull MenuItem item)
    {
        ...
        switch(id)
        {
            ...
            {
                this.startActivity(new Intent(this, CGovrnRule.class));
                super.finish();
                return true;
            }
